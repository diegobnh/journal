...
for index, row in df.iterrows():
     if(row['size_allocation'] > CHUNK_SIZE):
        ...
        while(i < total_obj):
           ...
           new_row=[row['ts_event_start'], row['mmap'], CHUNK_SIZE, new_start_addr, row['call_stack_hash'], row['call_stack_hexadecimal'], row['start_addr_decimal']]
           df.loc[df.shape[0]] = new_row
           i = i + 1;
        if(remnant_size > 0):
           new_start_addr = hex(row['start_addr_decimal'] + (i * CHUNK_SIZE))
           new_row=[row['ts_event_start'], row['mmap'], remnant_size, new_start_addr, row['call_stack_hash'], row['call_stack_hexadecimal'], row['start_addr_decimal']]
           df.loc[df.shape[0]] = new_row
...
output_filename="mmap_trace_by_chunk_" + app_dataset + ".csv"
