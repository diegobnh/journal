...
headers=["app_name","dram_store_30","dram_store_50","dram_store_70","pmem_store_30","pmem_store_50","pmem_store_70"]
df = pd.read_csv("store_ratio.csv", names=headers)
df.sort_values(by="app_name", inplace=True)
df = df[df.columns.drop(list(df.filter(regex='dram_')))]
...
df.plot(x="app_name", y=["pmem_store_30"], kind="bar", ax=axarr[0], color=['moccasin'])
df.plot(x="app_name", y=["pmem_store_50"], kind="bar", ax=axarr[1], color=['moccasin'])
df.plot(x="app_name", y=["pmem_store_70"], kind="bar", ax=axarr[2], color=['moccasin'])
...
axarr[0].legend(["NVM store"], ncol=2, loc='best', bbox_to_anchor =(1, 1.7))
...
fig.text(0.04, 0.5, 'Percentage of Store PMEM', va='center', rotation='vertical', fontsize=14)

axarr[2].set(xlabel=None)
filename = "store_ratio_in_PMEM.pdf"
