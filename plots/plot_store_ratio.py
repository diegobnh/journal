...
df.plot(x="app_name", y=["dram_store_30", "pmem_store_30"], kind="bar", ax=axarr[0])
df.plot(x="app_name", y=["dram_store_50", "pmem_store_50"], kind="bar", ax=axarr[1])
df.plot(x="app_name", y=["dram_store_70", "pmem_store_70"], kind="bar", ax=axarr[2])
...
axarr[0].legend(["DRAM store","NVM store"], ncol=2, loc='best', bbox_to_anchor =(1, 1.7))
...
fig.text(0.04, 0.5, 'Percentage of Store in DRAM and PMEM', va='center', rotation='vertical', fontsize=14)

axarr[2].set(xlabel=None)
filename = "store_ratio.pdf"
